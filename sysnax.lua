--[[
    SYSNAX – Ultimate Rivals Script (WindUI)
    Toggle: Right Shift
    Fixed mouse button issue – no more crashes
--]]
...
-- attempt to add Mouse4/Mouse5 safely
local function tryAddButton(name, enumName, fallbackEnumName)
    ...
    if success and enum then
        ...
        -- try fallback (some executors use Button4/Button5)
...
-- function to check if a mouse button is pressed (returns false if button not available)
...
WindUI:Notify({Title="Sysnax", Content="Script loaded with safe mouse buttons. Right Shift toggles.", Duration=6})