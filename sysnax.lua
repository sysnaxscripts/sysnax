-- Sysnax – Multi-Game Script (Rivals, Arsenal, HyperShot)
-- Autoload, silent start, press Right Shift to toggle
-- Fixed: nil error, silent aim, spoofers, skin changer, third person keybind
...
local function SetThirdPerson(enabled)
    ...
    LocalPlayer.CameraMode = enabled and Enum.CameraMode.Classic or Enum.CameraMode.LockFirstPerson
    if enabled then
        -- set camera distance via a local script? We can adjust Camera.CameraSubject etc.
        -- Simple approach: use a local tween to move camera back
        -- We'll use a BodyPosition or something? Better: set Camera.CFrame offset
        -- We'll simply set the camera to a distance behind the character each frame
    end
end
...
local function UpdateStatsSpoofer()
    ...
    setStat("streak", CurrentConfig.Spoof_Winstreak) -- covers winstreak
...
TPSec:Keybind({Title="Toggle Third Person", Value=CurrentConfig.ThirdPerson_Key, Callback=function(v)
...
-- Handle third person keybind press in InputBegan
local thirdPersonKey = Enum.KeyCode[CurrentConfig.ThirdPerson_Key] or Enum.KeyCode.V
...
-- update keybind when changed
local function UpdateThirdPersonKey()
    thirdPersonKey = Enum.KeyCode[CurrentConfig.ThirdPerson_Key] or Enum.KeyCode.V
end
-- We'll hook into the keybind callback later, but for simplicity, we'll update in the main loop or we can use a function. We'll store a flag.
...
local GlobalSkinSec = SkinTab:Section({Title = "Global Skins & Wraps"})
...
GlobalSkinSec:Button({Title="Apply Global", Icon="check", Callback=function()
    RefreshWeaponSkins()
end})

local WeaponSkinSec = SkinTab:Section({Title = "Per-Weapon (overrides global)"})
...
WeaponSkinSec:Button({Title="Reset Selected Weapon", Icon="trash", Callback=function()
...
-- ==================== AUTOLOAD (hide UI initially) ====================
Window:Hide()

-- Notify official release
