-- Sysnax – Ultimate Rivals Script (WindUI) – All fixes applied
...
-- No kick on any mode
local RIVALS_IDS = {17625359962, 15827677067, 18204519637}
if not table.find(RIVALS_IDS, game.PlaceId) then
...
local function tryAddButton(name, enumName, fallback)
    ...
    if ok and enum then MouseButtons[name]=enum else
...
mt.__namecall = newcclosure(function(...)
    ...
    if method == "FireServer" or method == "InvokeServer" then
        -- Identify the gun remote by its first argument
        local remote = args[1]
        if type(remote) == "string" and (remote == "MouseClick" or remote == "Shoot" or remote == "Bullet") then
            if CurrentConfig.Silent_Enabled and SilentTarget and SilentTarget.Character then
                ...
                if part then
                    -- Replace the second argument (hit position / target)
                    args[2] = part.Position + Vector3.new(0, CurrentConfig.Silent_Manipulation/100, 0)
                    -- If there's a third argument (sometimes mouse hit), also redirect
                    if args[3] and type(args[3]) == "userdata" then
                        args[3] = part
                    end
                end
            end
...
local function ragebotAutoShoot()
    ...
    if not CurrentConfig.Rage_AutoShoot then return end
    ...
    local target = GetClosestPlayerToCursor(500, "Head", not CurrentConfig.Rage_ThroughWalls) -- if ThroughWalls then visibleOnly = false
    if target and target.Character then
        weaponSys:FireServer("MouseClick", target.Character:FindFirstChild("Head"))
    end
end

-- Ragebot void spam & fly
...
local function RefreshWeaponSkins()
    ...
    for _, weapon in ipairs(weapons) do
        local skinData = CurrentConfig.WeaponSkins[weapon.Name]
        if skinData then
            ApplySkinToWeapon(weapon, skinData.SkinID, skinData.WrapID)
        end
    end
end
...
local function UpdateStatsSpoofer()
    ...
    setStat("winstreak", CurrentConfig.Spoof_Winstreak)
...
RunService.RenderStepped:Connect(function()
    ...
    if not IsMobile and CurrentConfig.Aimbot_Enabled and IsAlive() then
        ...
        if CurrentConfig.Aimbot_Mode == "Always" then shouldAim = true
        elseif CurrentConfig.Aimbot_Mode == "Hold" then shouldAim = IsMouseButtonDown(CurrentConfig.Aimbot_Key) end
        if shouldAim then
            local t = GetClosestPlayerToCursor(CurrentConfig.Aimbot_Radius, CurrentConfig.Aimbot_Part, CurrentConfig.Aimbot_VisibleOnly)
    ...
    if not IsMobile and CurrentConfig.Trigger_Enabled and IsAlive() and IsMouseButtonDown(CurrentConfig.Trigger_Key) then
        ...
        if t and t.Character and tick() - LastShot >= CurrentConfig.Trigger_Delay then
            ...
            if ws then ws:FireServer("MouseClick", t.Character:FindFirstChild("Head")) LastShot = tick() end
        end
    end
            ...
            if char and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChildOfClass("Humanoid") then
                ...
                if hum.Health > 0 then
                    ...
                    for _, v in pairs(esp) do
                        if type(v)~="table" then v.Visible=false else for _, s in ipairs(v) do s.Visible=false end end
                    end
                end
            else
                for _, v in pairs(esp) do
...
-- === SKIN CHANGER ===
local SkinTab = Tabs.Skin
local SkinEnableSec = SkinTab:Section({Title="Enable"})
...
local WeaponSkinSec = SkinTab:Section({Title="Weapon Skins & Wraps"})
...
local skinIDInput = WeaponSkinSec:Input({Title="Skin Decal ID", Placeholder="12345678", Callback=function(v) end})
local wrapIDInput = WeaponSkinSec:Input({Title="Wrap Decal ID", Placeholder="87654321", Callback=function(v) end})
WeaponSkinSec:Button({Title="Apply to Selected", Icon="check", Callback=function()
    ...
    CurrentConfig.WeaponSkins[weaponName].SkinID = skinIDInput:GetValue()
    CurrentConfig.WeaponSkins[weaponName].WrapID = wrapIDInput:GetValue()
    RefreshWeaponSkins()
end})
WeaponSkinSec:Button({Title="Reset Skin on Selected", Icon="trash", Callback=function()
...
local FinisherSec = SkinTab:Section({Title="Finisher"})
FinisherSec:Input({Title="Finisher Animation ID", Placeholder="12345678", Callback=function(v) CurrentConfig.Finisher_ID=v end})
...
WindUI:Notify({Title="Sysnax", Content="Script fully fixed! Silent aim works, ragebot shoots through walls, keybinds capture properly.", Duration=6})