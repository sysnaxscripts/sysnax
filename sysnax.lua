--[[
    SYSNAX – Ultimate Rivals Script
    UI: WindUI (gray/black theme)
    Logo: rbxassetid://122198206955790
    Toggle: Right Shift
    Features: Aimbot, Silent Aim, Triggerbot, Ragebot (enhanced),
              Visuals (ESP, Chams, Crosshair, Third Person),
              Skin Changer (Skin, Finisher, Charm, Wrap),
              Movement, Animations, Hit Sounds,
              Spoofer (Level, Rank, Winstreak, etc.),
              Leaderboard Spoofer (client‑side) with reset,
              Configs, Themes, Mobile support,
...
CurrentConfig = {
    ...
    -- Leaderboard Spoofer (client‑side)
    LBS_Enabled         = false,
    LBS_Wins            = "",
    LBS_Winstreak       = "",
    LBS_Level           = "",
    ...
    for _, part in ipairs(char:GetDescendants()) do
        if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then
            ...
            elseif cosmeticType == "Finisher" then
                -- Finisher is animation-based; just store ID
            end
        end
    end
end
local _lastSkin, _lastFinisher, _lastCharm, _lastWrap = "", "", "", ""
local function UpdateCosmetics()
    ...
    if CurrentConfig.Finisher_ID ~= _lastFinisher then
        ApplyCosmetic(LocalPlayer, CurrentConfig.Finisher_ID, "Finisher")
        _lastFinisher = CurrentConfig.Finisher_ID
    end
...
local LastVoid = 0
local LastShot = 0
local LastAimbotKeyState = false
...
-- === VISUALS ===
local ESPSec = Tabs.Visuals:Section("ESP")
...
ESPSec:Toggle({Title="Glow", Default=CurrentConfig.ESP_Glow, Callback=function(v) CurrentConfig.ESP_Glow=v end})
ESPSec:Toggle({Title="Outline", Default=CurrentConfig.ESP_Outline, Callback=function(v) CurrentConfig.ESP_Outline=v end})
ESPSec:Colorpicker({Title="ESP Color", Default=CurrentConfig.ESP_Color, Callback=function(v) CurrentConfig.ESP_Color=v end})

local ChamsSec = Tabs.Visuals:Section("Chams")
...
local CrossSec = Tabs.Visuals:Section("Crosshair")
...
local TPSec = Tabs.Visuals:Section("Third Person")
TPSec:Toggle({Title="Enable Third Person", Default=CurrentConfig.ThirdPerson_Enabled, Callback=function(v) CurrentConfig.ThirdPerson_Enabled=v end})
TPSec:Slider({Title="Distance", Step=1, Value={Min=5,Max=30,Default=CurrentConfig.ThirdPerson_Distance}, Callback=function(v) CurrentConfig.ThirdPerson_Distance=v end})

-- === RAGEBOT ===
local RageMainSec = Tabs.Ragebot:Section("Rage Options")
RageMainSec:Toggle({Title="Enable Ragebot", Default=CurrentConfig.Rage_Enabled, Callback=function(v) CurrentConfig.Rage_Enabled=v end})
RageMainSec:Toggle({Title="Instant Fire", Default=CurrentConfig.Rage_InstantFire, Callback=function(v) CurrentConfig.Rage_InstantFire=v end})
RageMainSec:Toggle({Title="Auto Shoot", Default=CurrentConfig.Rage_AutoShoot, Callback=function(v) CurrentConfig.Rage_AutoShoot=v end})
RageMainSec:Toggle({Title="No Recoil", Default=CurrentConfig.Rage_NoRecoil, Callback=function(v) CurrentConfig.Rage_NoRecoil=v end})
RageMainSec:Toggle({Title="No Spread", Default=CurrentConfig.Rage_NoSpread, Callback=function(v) CurrentConfig.Rage_NoSpread=v end})
RageMainSec:Toggle({Title="Void Spam", Default=CurrentConfig.Rage_VoidSpam, Callback=function(v) CurrentConfig.Rage_VoidSpam=v end})
RageMainSec:Slider({Title="Hide Time", Step=0.01, Value={Min=0.1,Max=2,Default=CurrentConfig.Rage_Hide}, Callback=function(v) CurrentConfig.Rage_Hide=v end})
RageMainSec:Slider({Title="Attack Time", Step=0.01, Value={Min=0.05,Max=1,Default=CurrentConfig.Rage_Attack}, Callback=function(v) CurrentConfig.Rage_Attack=v end})
RageMainSec:Toggle({Title="Fly", Default=CurrentConfig.Rage_Fly, Callback=function(v) CurrentConfig.Rage_Fly=v end})
RageMainSec:Slider({Title="Fly Speed", Step=1, Value={Min=10,Max=100,Default=CurrentConfig.Rage_FlySpeed}, Callback=function(v) CurrentConfig.Rage_FlySpeed=v end})
RageMainSec:Toggle({Title="NoClip", Default=CurrentConfig.Rage_NoClip, Callback=function(v) CurrentConfig.Rage_NoClip=v end})

-- === SKIN CHANGER ===
local SkinSec = Tabs.Skin:Section("Skin Changer")
SkinSec:Toggle({Title="Enable Skin Changer", Default=CurrentConfig.Skin_Enabled, Callback=function(v) CurrentConfig.Skin_Enabled=v end})
SkinSec:Input({Title="Skin ID", Value=CurrentConfig.Skin_ID, Callback=function(v) CurrentConfig.Skin_ID=v end})
SkinSec:Input({Title="Finisher ID", Value=CurrentConfig.Finisher_ID, Callback=function(v) CurrentConfig.Finisher_ID=v end})
SkinSec:Input({Title="Charm ID", Value=CurrentConfig.Charm_ID, Callback=function(v) CurrentConfig.Charm_ID=v end})
SkinSec:Input({Title="Wrap ID", Value=CurrentConfig.Wrap_ID, Callback=function(v) CurrentConfig.Wrap_ID=v end})

-- === MOVEMENT ===
local MoveGenSec = Tabs.Movement:Section("General")
MoveGenSec:Toggle({Title="Infinite Jump", Default=CurrentConfig.Move_InfiniteJump, Callback=function(v) CurrentConfig.Move_InfiniteJump=v end})
MoveGenSec:Toggle({Title="Fly", Default=CurrentConfig.Move_Fly, Callback=function(v) CurrentConfig.Move_Fly=v end})
MoveGenSec:Slider({Title="Fly Speed", Step=1, Value={Min=10,Max=100,Default=CurrentConfig.Move_FlySpeed}, Callback=function(v) CurrentConfig.Move_FlySpeed=v end})
MoveGenSec:Toggle({Title="Slide Boost", Default=CurrentConfig.Move_SlideBoost, Callback=function(v) CurrentConfig.Move_SlideBoost=v end})
MoveGenSec:Slider({Title="Boost Multiplier", Step=0.1, Value={Min=1,Max=5,Default=CurrentConfig.Move_SlideBoostMult}, Callback=function(v) CurrentConfig.Move_SlideBoostMult=v end})
MoveGenSec:Toggle({Title="Velocity", Default=CurrentConfig.Move_Velocity, Callback=function(v) CurrentConfig.Move_Velocity=v end})
MoveGenSec:Slider({Title="Velocity Speed", Step=1, Value={Min=1,Max=100,Default=CurrentConfig.Move_VelocitySpeed}, Callback=function(v) CurrentConfig.Move_VelocitySpeed=v end})
MoveGenSec:Toggle({Title="Underground", Default=CurrentConfig.Move_Underground, Callback=function(v) CurrentConfig.Move_Underground=v end})

-- === ANIMATIONS ===
local AnimSec = Tabs.Anims:Section("Custom Animation")
...
-- === HIT SOUNDS ===
local HitSec = Tabs.HitSound:Section("Hit Sound Options")
...
-- === SPOOFER ===
local SpoofStatsSec = Tabs.Spoofer:Section("Stats Spoofer")
...
local LBSec = Tabs.Spoofer:Section("Leaderboard Spoofer (only you see)")
...
local ThemeSec = Tabs.Settings:Section("Theme")
ThemeSec:Dropdown({Title="Select Theme", Values={"Sysnax","Dark","Light"}, Value="Sysnax", Callback=function(v) WindUI:SetTheme(v) end})

WindUI:Notify({Title="Sysnax", Content="Ultimate script loaded! Right Shift toggles.", Duration=6})