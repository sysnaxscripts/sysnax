-- Sysnax – Final Stable Release (Rivals / Arsenal / HyperShot)
-- UI: UwuWare 2.x (gray/black) · Right Shift toggle · Autoload · All modules

-- Safe library load (UwuWare is known to work on Synapse Z)
local Library = nil
...
if ok and type(err) == "table" and err.CreateWindow then
    Library = err
else
    game.Players.LocalPlayer:Kick("Sysnax failed to load UI. Try again.")
    return
end
...
-- Supported games (no kick, just notification)
local supported = {
...
local function ragebotUpdateTarget() if not Config.Rage_Enabled or not isAlive() then ragebotTarget=nil end; ragebotTarget = closestToCursor(1000, "Head", not Config.Rage_ThroughWalls) end
...
local SpooferTab = Window:AddTab("Spoofer")
...
SkinGlobal:AddButton("Apply to All Weapons", function() Config.WeaponSkins = {}; refreshAllSkins() end)

local SkinWeapon = SkinTab:AddSection("Weapon Customization")
local weaponCatDD = SkinWeapon:AddDropdown("Weapon Category", {"All","Melee","Primary","Secondary","Utility"}, "All", function() end)
local weaponListDD = SkinWeapon:AddDropdown("Select Weapon", {}, "", function(v)
...
local function updateWL()
    ...
    local cat = weaponCatDD:GetValue() or "All"
    local names = {}
...
SkinWeapon:AddButton("Refresh Weapons", updateWL)
updateWL()
...
SkinWeapon:AddButton("Apply to Selected", function()
    local weaponName = weaponListDD:GetValue()
...
-- Spoofer tab
local StatsSpoof = SpooferTab:AddSection("Stats Spoofer")
...
local NameSpoof = SpooferTab:AddSection("Name Spoof")
...
local LBGroup = SpooferTab:AddSection("Leaderboard Spoofer (client)")
...
LBGroup:AddButton("Reset", function()
...
local cfgInput = Configs:AddTextbox("Config Name", "", function() end)
Configs:AddButton("Save", function() saveCfg(cfgInput:GetText()) end)
Configs:AddButton("Load", function() loadCfg(cfgInput:GetText()) end)
Configs:AddButton("Delete", function() delCfg(cfgInput:GetText()) end)
local existingCfgs = listCfgs()
local cfgDrop = Configs:AddDropdown("Existing", existingCfgs, existingCfgs[1] or "", function(v) cfgInput:SetText(v) end)
...
-- Autoload hidden
Window:ToggleUI()
Window:Notify("🥳 Official Release  |  AC Bypassed  |  All modules active", 6)