-- Sysnax – Obsidian UI  |  Logo: rbxassetid://122198206955790
-- AC Bypassed  |  Desync  |  All features
-- Right Shift toggles  |  Autoload hidden
...
local function main()
    ...
    local Obsidian
    pcall(function()
        Obsidian = loadstring(game:HttpGet("https://raw.githubusercontent.com/deividcomsono/Obsidian/main/Obsidian.lua"))()
    end)
    ...
    CurrentConfig = {
        ...
        Aimbot_Radius=105, Aimbot_VisibleOnly=true, Aimbot_ShowFOV=true,
        Aimbot_FOVColor=Color3.fromRGB(255,255,255), Aimbot_Key="RightMouse",

        -- Legit > Bullet Redirection (Silent Aim)
    ...
    if silentFOV then
        silentFOV.Thickness = 2; silentFOV.NumSides = 48
        silentFOV.Filled = false; silentFOV.Transparency = 1; silentFOV.Visible = false
    end
            ...
            if CurrentConfig.Silent_Enabled and target and target.Character then
                ...
                if part then
                    if CurrentConfig.Hitbox_Enabled then
                        local hitboxPart = target.Character:FindFirstChild(CurrentConfig.Hitbox_Part)
                        if hitboxPart then
                            local originalSize = hitboxPart.Size
                            hitboxPart.Size = originalSize + Vector3.new(
                                CurrentConfig.Hitbox_ExtendRate/10,
                                CurrentConfig.Hitbox_ExtendRate/10,
                                CurrentConfig.Hitbox_ExtendRate/10
                            )
                            task.defer(function() task.wait(0.05); pcall(function() hitboxPart.Size = originalSize end) end)
                        end
                    end
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if silentFOV then
            ...
            silentFOV.Radius = CurrentConfig.Silent_FOVRadius
    ...
    -- ==================== UI TABS ====================
    local Legit   = Window:AddTab("Legit", "crosshair")
    local Rage    = Window:AddTab("Rage", "zap")
    local Visuals = Window:AddTab("Visuals", "eye")
    local Settings= Window:AddTab("Settings", "settings")

    -- ===== LEGIT =====
    local AimAssist = Legit:AddLeftGroupbox("Aim Assist", "crosshair")
    AimAssist:AddToggle("AimbotEnabled", {Text="Enabled", Default=CurrentConfig.Aimbot_Enabled, Callback=function(v) CurrentConfig.Aimbot_Enabled=v end})
    AimAssist:AddSlider("AimbotRadius", {Text="Aimbot FOV", Min=10,Max=360,Default=CurrentConfig.Aimbot_Radius, Rounding=0, Callback=function(v) CurrentConfig.Aimbot_Radius=v; if fovCircle then fovCircle.Radius=v end end})
    AimAssist:AddSlider("AimbotSmoothness", {Text="Smoothing Factor", Min=1,Max=25,Default=CurrentConfig.Aimbot_Smoothness, Rounding=1, Callback=function(v) CurrentConfig.Aimbot_Smoothness=v end})
    AimAssist:AddDropdown("AimbotPart", {Text="Hit Box", Values={"Head","HumanoidRootPart","LeftLeg","RightLeg"}, Default=CurrentConfig.Aimbot_Part, Callback=function(v) CurrentConfig.Aimbot_Part=v end})
    AimAssist:AddKeybind("AimbotKey", {Text="Aimbot Key", Default=CurrentConfig.Aimbot_Key, Callback=function(v) CurrentConfig.Aimbot_Key=v end})
    AimAssist:AddToggle("AimbotShowFOV", {Text="Draw Fov", Default=CurrentConfig.Aimbot_ShowFOV, Callback=function(v) CurrentConfig.Aimbot_ShowFOV=v end})
    AimAssist:AddSlider("AimbotNumSides", {Text="Num Sides", Min=4,Max=64,Default=48, Rounding=0, Callback=function(v) if fovCircle then fovCircle.NumSides=v end end})

    local BulletRedir = Legit:AddRightGroupbox("Bullet Redirection", "eye")
    BulletRedir:AddToggle("SilentEnabled", {Text="Enabled", Default=CurrentConfig.Silent_Enabled, Callback=function(v) CurrentConfig.Silent_Enabled=v end})
    BulletRedir:AddSlider("SilentFOV", {Text="Silent Aim FOV", Min=10,Max=360,Default=CurrentConfig.Silent_FOVRadius, Rounding=0, Callback=function(v) CurrentConfig.Silent_FOVRadius=v end})
    BulletRedir:AddSlider("SilentHitChance", {Text="Hit Chances", Min=0,Max=100,Default=CurrentConfig.Silent_HitChance, Rounding=0, Callback=function(v) CurrentConfig.Silent_HitChance=v end})
    BulletRedir:AddDropdown("SilentMode", {Text="Redirection Mode", Values={"P Mode"}, Default="P Mode", Callback=function() end})
    BulletRedir:AddDropdown("SilentPart", {Text="Hit Box", Values={"Head","HumanoidRootPart"}, Default=CurrentConfig.Silent_Part, Callback=function(v) CurrentConfig.Silent_Part=v end})
    BulletRedir:AddToggle("SilentDrawFOV", {Text="Draw Fov", Default=CurrentConfig.Silent_DrawFOV, Callback=function(v) CurrentConfig.Silent_DrawFOV=v end})
    BulletRedir:AddSlider("SilentNumSides", {Text="Num Sides", Min=4,Max=64,Default=48, Rounding=0, Callback=function(v) if silentFOV then silentFOV.NumSides=v end end})

    local ExtendHitbox = Legit:AddLeftGroupbox("Extend Hitbox", "maximize")
    ExtendHitbox:AddToggle("HitboxEnabled", {Text="Enabled", Default=CurrentConfig.Hitbox_Enabled, Callback=function(v) CurrentConfig.Hitbox_Enabled=v end})
    ExtendHitbox:AddDropdown("HitboxPart", {Text="Hit Box", Values={"Head","HumanoidRootPart"}, Default=CurrentConfig.Hitbox_Part, Callback=function(v) CurrentConfig.Hitbox_Part=v end})
    ExtendHitbox:AddSlider("HitboxRate", {Text="Extend Rate", Min=1,Max=50,Default=CurrentConfig.Hitbox_ExtendRate, Rounding=0, Callback=function(v) CurrentConfig.Hitbox_ExtendRate=v end})

    local TriggerBot = Legit:AddRightGroupbox("Trigger Bot", "zap")
    TriggerBot:AddToggle("TriggerEnabled", {Text="Enabled", Default=CurrentConfig.Trigger_Enabled, Callback=function(v) CurrentConfig.Trigger_Enabled=v end})
    TriggerBot:AddSlider("TriggerDelay", {Text="Delay (s)", Min=0,Max=1,Default=CurrentConfig.Trigger_Delay, Rounding=2, Callback=function(v) CurrentConfig.Trigger_Delay=v end})
    TriggerBot:AddSlider("TriggerDistance", {Text="Max Distance", Min=100,Max=500,Default=CurrentConfig.Trigger_Distance, Rounding=0, Callback=function(v) CurrentConfig.Trigger_Distance=v end})
    TriggerBot:AddKeybind("TriggerKey", {Text="Trigger Key", Default=CurrentConfig.Trigger_Key, Callback=function(v) CurrentConfig.Trigger_Key=v end})

    local RecoilCtrl = Legit:AddLeftGroupbox("Recoil Control", "minus")
    RecoilCtrl:AddToggle("RecoilEnabled", {Text="Enabled", Default=CurrentConfig.Recoil_Enabled, Callback=function(v) CurrentConfig.Recoil_Enabled=v end})
    RecoilCtrl:AddSlider("RecoilModel", {Text="Model Kick", Min=0,Max=100,Default=CurrentConfig.Recoil_ModelKick, Rounding=0, Callback=function(v) CurrentConfig.Recoil_ModelKick=v end})
    RecoilCtrl:AddSlider("RecoilCamera", {Text="Camera Kick", Min=0,Max=100,Default=CurrentConfig.Recoil_CameraKick, Rounding=0, Callback=function(v) CurrentConfig.Recoil_CameraKick=v end})
    ...
    local SkinGroup = Visuals:AddLeftGroupbox("Skin Changer", "shirt")
    SkinGroup:AddToggle("SkinEnabled", {Text="Enable Skin Changer", Default=CurrentConfig.Skin_Enabled, Callback=function(v) CurrentConfig.Skin_Enabled=v end})
    SkinGroup:AddInput("GlobalSkinID", {Text="Skin ID", Default=CurrentConfig.Global_SkinID, Callback=function(v) CurrentConfig.Global_SkinID=v end})
    SkinGroup:AddInput("GlobalWrapID", {Text="Wrap ID", Default=CurrentConfig.Global_WrapID, Callback=function(v) CurrentConfig.Global_WrapID=v end})
    SkinGroup:AddInput("GlobalFinisherID", {Text="Finisher ID", Default=CurrentConfig.Global_FinisherID, Callback=function(v) CurrentConfig.Global_FinisherID=v end})
    SkinGroup:AddButton("ApplyGlobal", {Text="Apply Globally", Callback=refreshSkins})

    -- ===== SETTINGS =====
    local KeybindGroup = Settings:AddLeftGroupbox("Keybinds", "settings")
    ...
    ConfigGroup:AddButton("SaveConfig", {Text="Save", Callback=function() saveCfg(cfgInput.Value) end})
    ConfigGroup:AddButton("LoadConfig", {Text="Load", Callback=function() loadCfg(cfgInput.Value) end})
    ConfigGroup:AddButton("DeleteConfig", {Text="Delete", Callback=function() delCfg(cfgInput.Value) end})
    local cfgList = listCfgs()
    local cfgDropdown = ConfigGroup:AddDropdown("ExistingConfigs", {Text="Configs", Values=cfgList, Default=cfgList[1] or "", Callback=function(v) cfgInput:SetValue(v) end})
    ConfigGroup:AddButton("RefreshConfigs", {Text="Refresh", Callback=function() cfgDropdown:Refresh(listCfgs()) end})

    local ThemeGroup = Settings:AddLeftGroupbox("Theme", "palette")
    ThemeGroup:AddDropdown("ThemeSelect", {Text="Theme", Values={"Sysnax","Dark","Light"}, Default="Sysnax", Callback=function(v)
        if v == "Sysnax" then
            Window:SetTheme({
                Background = Color3.fromRGB(25,25,30),
                Tab = Color3.fromRGB(35,35,40),
